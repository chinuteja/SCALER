import sys
# def remove(A,left_most,right_most):
#     N = len(A)
#     if left_most == 0:
#         left_sum = 0
#         right_sum = sum(A[N-right_most:])
#         return left_sum + right_sum

#     elif right_most == 0:
#         right_sum = 0
#         left_sum = sum(A[:left_most])
#         return left_sum + right_sum
#     else:
#         left_sum = sum(A[:left_most])
#         right_sum = sum(A[N-right_most:])
...
n = len(A)
B = 118
prefix_array = [0 for i in range(n)]
suffix_array = [0 for i in range(n)]
...
i = len(A)-2
while i >=0:
    suffix_array[i] = suffix_array[i+1] + A[i]
    i = i - 1


max_sum = max(prefix_array[B-1],suffix_array[n-B])
# max_sum = -sys.maxsize
# print(max_sum)
for i in range(1,B):
    req_sum = prefix_array[i-1] + suffix_array[n-B+i] #5-3+3

    max_sum = max(max_sum,req_sum)
